...
contract Universe is DelegationTarget, ITyped, IUniverse {
    ...
    function createYesNoMarket(uint256 _endTime, uint256 _feePerEthInWei, ERC20 _denominationToken, address _designatedReporterAddress, bytes32 _topic, string _description, string _extraInfo) public onlyInGoodTimes payable returns (IMarket _newMarket) {
    ...
    function createCategoricalMarket(uint256 _endTime, uint256 _feePerEthInWei, ERC20 _denominationToken, address _designatedReporterAddress, bytes32[] _outcomes, bytes32 _topic, string _description, string _extraInfo) public onlyInGoodTimes payable returns (IMarket _newMarket) {
        require(bytes(_description).length > 0);
    ...
    function createScalarMarket(uint256 _endTime, uint256 _feePerEthInWei, ERC20 _denominationToken, address _designatedReporterAddress, int256 _minPrice, int256 _maxPrice, uint256 _numTicks, bytes32 _topic, string _description, string _extraInfo) public onlyInGoodTimes payable returns (IMarket _newMarket) {
        require(bytes(_description).length > 0);
    ...
    function createMarketInternal(uint256 _endTime, uint256 _feePerEthInWei, ERC20 _denominationToken, address _designatedReporterAddress, address _sender, uint256 _numOutcomes, uint256 _numTicks) private onlyInGoodTimes returns (IMarket _newMarket) {
        MarketFactory _marketFactory = MarketFactory(controller.lookup("MarketFactory"));
        _newMarket = _marketFactory.createMarket(controller, this, _endTime, _feePerEthInWei, _denominationToken, _designatedReporterAddress, _sender, _numOutcomes, _numTicks);
