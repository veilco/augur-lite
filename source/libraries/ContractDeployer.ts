...
`);
    ...
    private static getEncodedConstructData(
        ...
        const constructorInputTypes = constructorSignature.inputs.map(
            x => x.type
        );
    ...
    private async upload(contract: Contract): Promise<void> {
        ...
        if (
            ...
            contract.address = await this.getExistingContractAddress(
                contractName
            );
        } else {
            console.log(
                `Uploading new version of contract for ${contractName}`
            );
    ...
    private async whitelistTradingContracts(): Promise<void> {
        ...
        for (let contract of this.contracts) {
            ...
            if (await this.controller.whitelist_(contract.address)) {
                console.log(
                    `Skipping already whitelisted ${contract.contractName}.`
                );
                continue;
            } else {
    ...
    private async initializeContract(
        ...
        await this.getContract(contractName).setController(
            this.controller.address
        );
    }
