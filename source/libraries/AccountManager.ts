...
export class AccountManager {
  ...
  public async getNonce(address: string): Promise<BN> {
    if (!this.accounts.has(address))
      throw new Error(
        `Nonce requested for an account not managed by this Account Manager.  Requested address: ${address}`
      );
  ...
  public async signTransaction(transaction: Transaction): Promise<string> {
    ...
    if (typeof transaction.data === "undefined")
      throw new Error(`transaction.data was undefined.`);
    if (typeof transaction.gas === "undefined")
      throw new Error(`transaction.gas was undefined.`);
    if (typeof transaction.gasPrice === "undefined")
      throw new Error(`transaction.gasPrice was undefined.`);
