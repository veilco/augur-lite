...
export class ContractInterfaceGenerator {
  ...
  public async generateContractInterfaces(): Promise<String> {
    ...
    const fileContents: String = this.contractInterfacesTemplate(
      contractsOutput.contracts
    );
    await fs.writeFile(
      this.configuration.contractInterfacesOutputPath,
      fileContents
    );
    return fileContents;
  }
  ...
  private contractInterfacesTemplate(contracts: CompilerOutputContracts) {
    ...
    contractInterfaces.push(
      this.contractInterfaceTemplate(
        "Controlled",
        contracts["Controlled.sol"]["Controlled"].abi
      )
    );
    ...
    for (let globalName in contracts) {
      for (let contractName in contracts[globalName]) {
        ...
        contractInterfaces.push(
          this.contractInterfaceTemplate(contractName, contractAbi)
        );
      }
    }
  ...
  private contractInterfaceTemplate(contractName: String, contractAbi: Abi) {
    ...
    const extendsControlled: boolean =
      seen.has("getController") && contractName != "Controlled";

    return `
    export class ${contractName} extends ${
      extendsControlled ? "Controlled" : "Contract"
    } {
...
`;
  ...
  private remoteMethodTemplate(abiFunction: AbiFunction) {
    ...
    const options: String = `{ sender?: string, gasPrice?: BN${
      abiFunction.payable ? ", attachedEth?: BN" : ""
    } }`;
    return `        public ${
      abiFunction.name
            ...
            await this.remoteCall(abi, [${argNames}], "${
      abiFunction.name
    }", options.sender, options.gasPrice${
      abiFunction.payable ? ", options.attachedEth" : ""
    });
            return;
        }`;
  }
  ...
  private localMethodTemplate(abiFunction: AbiFunction) {
    const argNames: String = this.getArgNamesString(abiFunction);
    const params: String = this.getParamsString(abiFunction);
    const options: String = `{ sender?: string${
      abiFunction.payable ? ", attachedEth?: BN" : ""
    } }`;
    const returnType: String =
      abiFunction.outputs[0] !== undefined
        ? this.getTsTypeFromPrimitive(abiFunction.outputs[0].type)
        : "void";
    const returnPromiseType: String =
      abiFunction.outputs.length === 0 || abiFunction.outputs.length === 1
        ? returnType
        : "Array<string>";
    const returnValue: String =
      abiFunction.outputs.length == 1
        ? `<${returnType}>result[0]`
        : "<Array<string>>result";
    return `        public ${
      abiFunction.name
    }_ = async(${params} options?: ${options}): Promise<${returnPromiseType}> => {
            options = options || {};
            const abi: AbiFunction = ${JSON.stringify(abiFunction)};
            ${
              abiFunction.outputs.length !== 0 ? "const result = " : ""
            }await this.localCall(abi, [${argNames}], options.sender${
      abiFunction.payable ? ", options.attachedEth" : ""
    });
  ...
  private getParamsString(abiFunction: AbiFunction) {
    ...
    return (
      abiFunction.inputs
        .map(
          (v, i) =>
            (v.name.substr(1) || `arg${i}`) +
            ": " +
            this.getTsTypeFromPrimitive(v.type)
        )
        .join(", ") + ","
    );
  }
