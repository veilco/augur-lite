...
export class ContractCompiler {
    ...
    public async compileContracts(): Promise<CompilerOutput> {
        ...
        const compilerOutput = await this.compileCustomWrapper(
            compilerInputJson
        );
        ...
        const filteredCompilerOutput = this.filterCompilerOutput(
            compilerOutput
        );
        await fs.writeFile(
            this.configuration.contractOutputPath,
    ...
    public async generateFlattenedSolidity(filePath: string): Promise<string> {
        ...
        const childProcess = exec(
            format(this.flattenerCommand, relativeFilePath),
            {
                encoding: "buffer",
        ...
        for (let relativeFilePath in compilerOutput.contracts) {
            for (let contractName in compilerOutput.contracts[
                ...
                if (
                    ...
                    continue;
                const abi =
                    compilerOutput.contracts[relativeFilePath][contractName]
                        .abi;
